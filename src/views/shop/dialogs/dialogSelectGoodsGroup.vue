    ...
    transTreeData(data, pid) {
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i].parentId == pid) {
                var obj = {"categoryName": data[i].name,"id": data[i].id, parentId: data[i].parentId, level: data[i].level, sort: data[i].sort, image: data[i].image, enable: data[i].enable, goods: (data[i].goods || [])};
      ...
      return (
        <div class="treeRow">
          ...
          <span class="td operate">
              {
                  <span class="blue" on-click={() => this.change(node, data)}>选择商品</span>
              }
          </span>
        </div>
      )
